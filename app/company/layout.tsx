...
export default function CompanyLayout({
  ...
  let navigation = [
    ...
    { name: "Finances", href: "/company/finances", icon: Wallet },
          ...
          <aside className="relative flex-1 max-w-[280px] w-full bg-white flex flex-col h-full transform transition-transform duration-300 ease-in-out">
            <div className="h-16 flex items-center px-6 border-b border-gray-100 shrink-0">
              <div className="flex items-center space-x-2 cursor-default">
                <Bus className="w-6 h-6" style={{ color: companyInfo?.color || '#059669' }} />
                <span className="text-xl font-black text-gray-900 tracking-tight">
                  TUKKI <span className="text-xs uppercase tracking-widest block font-bold" style={{ color: companyInfo?.color || '#059669' }}>Partner</span>
                </span>
              </div>
              <button 
            ...
            <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto custom-scrollbar">
              {navigation.map((item) => {
                ...
                return (
                  <Link
                    ...
                    style={isActive ? { backgroundColor: companyInfo?.color || '#059669' } : {}}
                  >
              ...
              <button onClick={handleLogout} className="flex items-center space-x-3 px-3 py-2.5 w-full rounded-xl font-medium text-red-600 hover:bg-red-50 transition-colors mt-2">
